...
public class ASTBuilder extends SFMLBaseVisitor<ASTNode> {
    ...
    public ASTNode visitResource(SFMLParser.ResourceContext ctx) {
        ...
        ResourceIdentifier<Object, Object, Object> rtn = ResourceIdentifier.fromString(str);
    ...
    public ResourceIdentifier<?, ?, ?> visitStringResource(SFMLParser.StringResourceContext ctx) {
        ResourceIdentifier<Object, Object, Object> rtn = ResourceIdentifier.fromString(visitString(ctx.string()).value());
        USED_RESOURCES.add(rtn);
        rtn.assertValid();
        AST_NODE_CONTEXTS.add(new Pair<>(rtn, ctx));
        return rtn;
    ...
    public Label visitStringLabel(SFMLParser.StringLabelContext ctx) {
        Label label = new Label(visitString(ctx.string()).value());
        if (label.name().length() > Program.MAX_LABEL_LENGTH) {
            throw new IllegalArgumentException(
                    "Label name cannot be longer than "
                    + Program.MAX_LABEL_LENGTH
    ...
    public Program visitProgram(SFMLParser.ProgramContext ctx) {
        if (SFMServerConfig.disableProgramExecution) {
            throw new AssertionError("Program execution is disabled via config");
        }
        StringHolder name = visitName(ctx.name());
        List<Trigger> triggers = ctx
                .trigger()
                .stream()
                .map(this::visit)
                .map(Trigger.class::cast)
                .collect(Collectors.toList());
        Set<String> labels = USED_LABELS
                .stream()
                .map(Label::name)
    ...
    public ASTNode visitTimerTrigger(SFMLParser.TimerTriggerContext ctx) {
        ...
        Interval time = (Interval) visit(ctx.interval());
        ...
        int minInterval = timerTrigger.usesOnlyForgeEnergyResourceIO()
                          ? SFMServerConfig.timerTriggerMinimumIntervalInTicksWhenOnlyForgeEnergyIO
                          : SFMServerConfig.timerTriggerMinimumIntervalInTicks;
    ...
    public InputStatement visitInputStatement(SFMLParser.InputStatementContext ctx) {
        LabelAccess labelAccess = visitLabelAccess(ctx.labelAccess());
        ResourceLimits matchers = visitInputResourceLimits(ctx.inputResourceLimits());
        ResourceIdSet exclusions = visitResourceExclusion(ctx.resourceExclusion());
    ...
    public OutputStatement visitOutputStatement(SFMLParser.OutputStatementContext ctx) {
        LabelAccess labelAccess = visitLabelAccess(ctx.labelAccess());
        ResourceLimits matchers = visitOutputResourceLimits(ctx.outputResourceLimits());
        ResourceIdSet exclusions = visitResourceExclusion(ctx.resourceExclusion());
        boolean each = ctx.EACH() != null;
    ...
    public IfStatement visitIfStatement(SFMLParser.IfStatementContext ctx) {
        ArrayDeque<BoolExpr> conditions = ctx
                .boolexpr()
                .stream()
                .map(this::visit)
                .map(BoolExpr.class::cast)
                .collect(Collectors.toCollection(ArrayDeque::new));
        ArrayDeque<Block> blocks = ctx.block().stream()
                .map(this::visitBlock)
                .collect(Collectors.toCollection(ArrayDeque::new));
        ...
        while (!blocks.isEmpty()) {
            nestedStatement = new IfStatement(
                    conditions.removeLast(),
                    blocks.removeLast(),
                    new Block(Collections.singletonList(nestedStatement))
            );
        }
    ...
    public BoolExpr visitBooleanHas(SFMLParser.BooleanHasContext ctx) {
        SetOperator setOperator = visitSetOp(ctx.setOp());
        LabelAccess labelAccess = visitLabelAccess(ctx.labelAccess());
    ...
    public BoolExpr visitBooleanConjunction(SFMLParser.BooleanConjunctionContext ctx) {
        BoolExpr left = (BoolExpr) visit(ctx.boolexpr(0));
        BoolExpr right = (BoolExpr) visit(ctx.boolexpr(1));
    ...
    public Limit visitQuantityRetentionLimit(SFMLParser.QuantityRetentionLimitContext ctx) {
        ResourceQuantity quantity = visitQuantity(ctx.quantity());
        ResourceQuantity retain = visitRetention(ctx.retention());
    ...
    public ResourceIdSet visitResourceExclusion(@Nullable SFMLParser.ResourceExclusionContext ctx) {
        ...
        ResourceIdSet resourceIdSet = visitResourceIdList(ctx.resourceIdList());
    ...
    public ResourceLimits visitInputResourceLimits(@Nullable SFMLParser.InputResourceLimitsContext ctx) {
        if (ctx == null) {
            return new ResourceLimits(Collections.singletonList(ResourceLimit.TAKE_ALL_LEAVE_NONE), ResourceIdSet.EMPTY);
        }
    ...
    public ResourceLimits visitOutputResourceLimits(@Nullable SFMLParser.OutputResourceLimitsContext ctx) {
        if (ctx == null) {
            return new ResourceLimits(Collections.singletonList(ResourceLimit.ACCEPT_ALL_WITHOUT_RESTRAINT), ResourceIdSet.EMPTY);
        }
    ...
    public WithConjunction visitWithConjunction(SFMLParser.WithConjunctionContext ctx) {
        WithClause left = (WithClause) visit(ctx.withClause(0));
        WithClause right = (WithClause) visit(ctx.withClause(1));
    ...
    public WithParen visitWithParen(SFMLParser.WithParenContext ctx) {
        WithClause inner = (WithClause) visit(ctx.withClause());
